...
log_path = "checkpoints/training_logs_v7.csv"
if not os.path.exists(log_path):
    with open(log_path, "w") as f:
        f.write("step,critic_loss,actor_loss,entropy_alpha,step_reward\n")

# ---- Resume + curriculum setup ----
RESUME_CHECKPOINT = "checkpoints/checkpoint_v6_final_749999.pt"  # <-- confirm exact filename
START_STEP = 750000
STAGE_STEPS = 500000  # how long this stage runs
nav_scale = 0.1


def get_curriculum_penalties(global_step, start_step=750000, base_out_of_road=15.0, base_crash=15.0,
                              escalation_step=250000, max_multiplier=3.0):
    steps_into_curriculum = global_step - start_step
    stage = steps_into_curriculum // escalation_step
    multiplier = min(1.0 + stage * 0.5, max_multiplier)
    return base_out_of_road * multiplier, base_crash * multiplier


out_of_road_penalty, crash_vehicle_penalty = get_curriculum_penalties(START_STEP, start_step=START_STEP)
print(f"Stage starting at step {START_STEP}: out_of_road_penalty={out_of_road_penalty}, "
      f"crash_vehicle_penalty={crash_vehicle_penalty}")
...
def load_demonstrations(replay_buffer, demo_path="demonstrations.pkl"):
    ...
    for transition in demos:
        replay_buffer.add(
            transition["obs_grid"], transition["obs_num_others"], transition["obs_nav_info"],
            transition["action"], transition["reward"],
            transition["next_obs_grid"], transition["next_obs_num_others"], transition["next_obs_nav_info"],
...
# ---- Networks: construct, then load resumed weights ----
...
resume_ckpt = torch.load(RESUME_CHECKPOINT, map_location=torch.device('cpu'))
cnn_extractor.load_state_dict(resume_ckpt["cnn_extractor"])
actor.load_state_dict(resume_ckpt["actor"])
critic.load_state_dict(resume_ckpt["critic"])
...
cnn_optimizer = torch.optim.Adam(cnn_extractor.parameters(), lr=1e-4)
...
print("\n" + "=" * 60)
print(f"LAUNCHING TRAINING PIPELINE (v7 - resumed from {START_STEP}, curriculum stage)")
print("=" * 60)

for global_step in range(START_STEP, START_STEP + STAGE_STEPS):
    ...
    modified_reward = reward  # comfort penalty stays off, per last confirmed test
    ...
    if (global_step + 1) % 50000 == 0:
        ...
        torch.save(checkpoint, f"checkpoints/checkpoint_v7_{global_step}.pt")
...
torch.save(final_checkpoint, f"checkpoints/checkpoint_v7_final_{global_step}.pt")
